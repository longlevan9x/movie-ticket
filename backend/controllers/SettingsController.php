        ...
        if($request->isAjax){
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                    FHtml::saveModel($modelMeta, $request->post(), ['settings_id' => $model->id]);

                    $files =FHtml::getUploadedFiles($model, $uploadFields, 'settings' . FHtml::getAttribute($model, 'id'), $oldModel);

                    FHtml::saveFiles($files, $this->uploadFolder .  '/settings/', $model);
        ...
        if($request->isAjax){
            ...
            */
            ...
            if ($model->load($request->post())) {
                ...
                if ($model->save()) {
                        FHtml::saveFiles($files, $this->uploadFolder .  '/settings/');
                        FHtml::saveModel($modelMeta, $request->post(), ['settings_id' => $model->id]);
                        FHtml::saveObjectItems($model, $this->moduleKey, $model->id, $model::getRelatedObjects());
                        if ($this->saveType() == 'clone')
                        {
