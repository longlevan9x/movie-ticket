...
class Paypal extends Component
{
    ...
    */
    ...
    public function init()
    {
        ...
        $this->returnUrl = Yii::$app->urlManager->createAbsoluteUrl($this->returnUrl);
    ...
    public function SetExpressCheckout($paymentInfo = array())
    {
        ...
        $resArray = $this->hash_call("SetExpressCheckout", $nvpstr, $desc);
        return $resArray;
    }

    public function SetRecurringPayment($paymentInfo = array())
    {
        $amount = urlencode($paymentInfo['Order']['theTotal']);
        //description
        if ($paymentInfo['Order']['description']) {
    ...
    public function hash_call($methodName, $nvpStr, $description = '', $username = '', $password = '', $signature = '')
    {
        $API_UserName = !empty($username) ? $username : FHtml::paypalAPIUsername();//$this->apiUsername;
        $API_Password = !empty($password) ? $password : FHtml::paypalAPIPassword();
        $API_Signature = !empty($signature) ? $signature : FHtml::paypalAPISignature();
        $API_Endpoint = $this->endPoint;
        $description = !empty($description) ? $description : $this->defaultDescription;
        ...
        $nvpreq = "METHOD=" . urlencode($methodName) . "&VERSION=" . urlencode('86') . "&PWD=" . urlencode($API_Password) . "&USER=" . urlencode($API_UserName) . "&SIGNATURE=" . urlencode($API_Signature);
        $nvpreq .= '&L_BILLINGTYPE0=' . urlencode('RecurringPayments') . '&L_BILLINGAGREEMENTDESCRIPTION0=' . urlencode($description) . '';
        $nvpreq .= '&cancelUrl=' . urlencode($this->cancelUrl);
        $nvpreq .= '&returnUrl=' . urlencode($this->returnUrl);
