...
<div class="row high-rated margin-bottom-20">
    ...
    if ($count_gallery != 0) {
        ...
        ?>
        <!-- Begin Easy Block -->
        <div class="col-md-<?= $item_side ?> col-sm-6 md-margin-bottom-20" style="padding-right:20px">
            <div class="easy-block-v2" style="background-color: white">
                <a href='<?= $linkurl ?>'>
                    <div class="easy-bg-v2 rgba-<?= $type_color ?>"><?= $type ?></div>
                    <div class="tinto" style="height: auto; max-height: 300px;">
                        <?= FHtml::showImage($item->image, $image_folder, $image_width, $image_height, 'img-responsive1') ?>
                    </div>
                    <div style="padding:20px; height: 180px">
                        <h3 style="font-weight: bold; color: #81C6B6"><?= FHtml::showLinkUrl(FHtml::getFieldValue($item, $field_title), $linkurl, true) ?></h3>
                        <?= FHtml::showModelPrice($item, $color, false, $show_price) ?>
                        <ul class="list-unstyled">
                            <?= FHtml::showModelRates($item, 5, $color, $show_rate) ?>
                            <?= FHtml::showModelField($item, $field_overview, '', $item_layout, '', '', 300) ?>
                        </ul>
                    </div>
                </a>
                ...
                <?php if ($show_viewmore) { ?>
                    <div class="nutbt">
                        <button class="btn"><a href="<?= $linkurl ?>" style="color:white !important;"><?= FHtml::t('common', 'Read more') ?> <i
                                    class="fa fa-long-arrow-right home-a1" aria-hidden="true"></i>
                            </a></button>
                    </div>
                <?php } ?>
            </div>
