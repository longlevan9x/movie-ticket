...
class ActiveField extends Component
{
    ...
    public function numeric($options = [], $class = FHtml::EDITOR_NUMERIC)
    {
        $options = array_merge($options, ['clientOptions' => ['alias' =>  'numeric', 'groupSeparator' => ',', 'autoGroup' => true, 'removeMaskOnSubmit' => true]]);

        return self::widget($class, $options);
    }

    public function numericInput($options = [], $class = FHtml::EDITOR_NUMERIC)
    {
    ...
    public function selectCategory() {
        return self::select('object_category');
    }

    public function lookup($object_type, $options = [], $populated_fields = [], $name_field = 'name', $id_field = 'id') {
        return self::select($object_type, $options, $object_type, $populated_fields, $name_field, $id_field);
    }
}
